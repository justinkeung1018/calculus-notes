...
\begin{definition}
  ...
  \[
    \forall \epsilon > 0, \exists N \in \N (\forall n, m \in \N (n > N \land m > N \implies \abs{a_n - a_m} < \epsilon))
  \]
\end{definition}
The need to specify that the Cauchy sequence is in the real numbers will become apparent later.
\begin{prop}
  A sequence $(a_n)_{n \geq 1}$ converges to a limit $l \in \R$ (note $\R$) $\implies$ $a_n$ is a Cauchy sequence (in the real numbers).
\end{prop}
\begin{proof}
  Since $a_n$ converges to $l$, 
  \[
    \forall \epsilon > 0, \exists N \in \N (\forall n \in \N (n > N \implies \abs{a_n - l} < \epsilon))
  \]
  Given some $\epsilon > 0$, 
  \[
    \exists N' \in \N (\forall n \in \N (n > N' \implies (\abs{a_n - l} < \frac{\epsilon}{2} < \epsilon)))
  \]
  Choose arbitrary $n, m > N'$. Then $\abs{a_n - l} < \frac{\epsilon}{2}$ and $\abs{a_m - l} < \frac{\epsilon}{2}$.
  \begin{align*}
...
\end{explanation}
\begin{prop}
  Let $(a_n)_{n \geq 1}$ be a Cauchy sequence. Then $a_n$ is bounded.
\end{prop}
\begin{proof}
  By definition,
  \[
    \forall \epsilon > 0, \exists N \in \N \colon \forall n, m \in \N (n > N \land m > N \implies \abs{a_n - a_m} < \epsilon)
  \]
  Let
  \[
    M = \epsilon + \max{\abs{a_1}, \abs{a_2}, \abs{a_3}, ..., \abs{a_{N + 1}}}
  \] 
  Then $\forall n > N$,
  \begin{align*}
